...
from tile_roi_with_labels import tile_roi_with_labels, random_crop_roi


def tile_roi_with_labels_wrapper(
    fps: List[str], save_dir: Union[str, List[str]], tile_size: int, 
...
def random_crop_roi_wrapper(
    fps: List[str], save_dir: Union[str, List[str]], crop_size: int, nproc: int = 10,
    fill: Tuple[int] = (114, 114, 114), box_thr: float = 0.5, 
    notebook: bool = False
) -> DataFrame:
    """
    ...
    with Pool(nproc) as pool:
        jobs = [
            pool.apply_async(
                ...
                args=(fp, sd, crop_size, box_thr, fill)) 
            for fp, sd in zip(fps, save_dir)]
